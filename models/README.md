...
Both models are this project's own work and are licensed under
[CC BY 4.0](https://creativecommons.org/licenses/by/4.0/), separately from the code
(`MIT OR Apache-2.0`). You may use, share and adapt them, commercially too, provided you give
credit, link the licence, and say whether you changed them. Credit them as:

> SCRFD-80k face detector by Face Crop Studio Contributors,
> https://github.com/gregorycarnegie/face-crop-studio, licensed under CC BY 4.0

> Face Crop Studio eye refiner by Face Crop Studio Contributors,
> https://github.com/gregorycarnegie/face-crop-studio, licensed under CC BY 4.0

The same line travels inside each file: `tools/dataset/stamp_model_metadata.py` writes it to
the ONNX metadata as `attribution`, alongside `source`, `license`, `author`, `training_data`
and a description of the inputs and outputs. Netron shows these on opening the file, and ONNX
Runtime returns them from `get_modelmeta()`. Both exporters stamp before they verify, so a new
export carries them too. The fields are metadata only: the graph and weights are unchanged.

The training photographs are Open Images, CC BY 2.0, credited in the attribution manifest
described under each model.

## Eye refiner

- **File**: `eye_refiner.onnx`
  - **SHA256**: `16b4ed6e42889c0eb3d6ce5068e399c19f12ad686bd121bcf959db34f8cef976`
  ...
  - **Runtime**: built-in Rust CPU graph, with no ONNX Runtime library required. Convolution/ReLU reuse the detector's kernels, global average pooling averages each channel plane, and the two dense layers reuse 1x1 convolution. The fixed topology and initializer names in `fcs-core/src/eye_refiner/native.rs` match this export; re-exported models must pass the native parity check. A missing or incompatible model leaves the detector's own landmarks in place.
...
- **File**: `scrfd80k_500m_640.onnx`
  - **SHA256**: `2437def9510f2af6431a5b2090ae9f357bc189cb0223be1b6c4d4ff085798e64`
  ...
  - **Runtime**: WGSL kernels or the built-in Rust CPU graph, checked against the development-only ONNX Runtime oracle to about 1e-05. No runtime library is bundled or loaded by the application. The weights are read by name, which is why the export folds BatchNorm itself rather than letting torch's constant folding rename them. Run at score 0.5.

// Candidate for experiment 45: f16 storage *and* f16 arithmetic, in 41's register form.
// Run with --f16-storage under DXC (FXC has no f16). Output converts to f32 at the write.
enable f16;
...
fn load4(ox_start: u32, base: u32) -> vec4<f16> {
    var values = vec4<f16>(0.0);
...
fn full_tile(ox_start: u32, oy: u32, oc: u32) {
    ...
    let w2 = w1 + c_in;
    let w3 = w2 + c_in;
    var a0 = vec4<f16>(bias[oc]);
    var a1 = vec4<f16>(bias[oc + 1u]);
    var a2 = vec4<f16>(bias[oc + 2u]);
    var a3 = vec4<f16>(bias[oc + 3u]);
    for (var ic = 0u; ic < c_in; ic++) {
        let values = load4(ox_start, ic * plane + pixel);
        a0 = fma(values, vec4<f16>(weights[w0 + ic]), a0);
        a1 = fma(values, vec4<f16>(weights[w1 + ic]), a1);
        a2 = fma(values, vec4<f16>(weights[w2 + ic]), a2);
        a3 = fma(values, vec4<f16>(weights[w3 + ic]), a3);
    }
...
fn one_channel(ox_start: u32, oy: u32, oc: u32) {
    ...
    let plane = params.input_width * params.input_height;
    let pixel = oy * params.input_width + ox_start;
    var a = vec4<f16>(bias[oc]);
    for (var ic = 0u; ic < c_in; ic++) {
        a = fma(load4(ox_start, ic * plane + pixel), vec4<f16>(weights[oc * c_in + ic]), a);
    }

...
`ScrfdDetector::load_from` tries the WGSL compute kernels, then the built-in CPU graph, and
reports which one won in the startup log (`Detector: SCRFD-80k on wgsl-gpu` or `cpu-graph`).
